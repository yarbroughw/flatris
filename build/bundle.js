...
/***/ function(module, exports, __webpack_require__) {
	...
	var Piece = function () {
	    ...
	    _createClass(Piece, [{
	        ...
	        get: function get() {
	            ...
	            return this.layout.map(function (space) {
	                return Space(space.x + _this.x, space.y + _this.y, _this.color);
	            });
	        }
	    }]);
	...
	function inXBounds(piece, gridwidth) {
	    var bools = piece.spaces.map(function (square) {
	        return 0 <= square.x && square.x < gridwidth;
	    });
	    return bools.indexOf(false) == -1;
	}

	function colliding(piece, board) {
	    var colliding = function colliding(space) {
	        return function (boardsquare) {
	            return boardsquare.x == space.x && boardsquare.y == space.y;
	        };
	    };
	...
	function shiftPiece(piece, board, dx, dy) {
	    var newPiece = piece.shifted(dx, dy);
	...
	function clearRows(board) {
	    ...
	    var notInRow = function notInRow(row) {
	        return function (space) {
	            return space.y != row;
	        };
	    };
	    var bumpSpaceDown = function bumpSpaceDown(row) {
	        return function (space) {
	            return space.y < row ? new Space(space.x, space.y + 1, space.color) : space;
	        };
	    };
	...
	function mainState() {
	    var state = arguments.length <= 0 || arguments[0] === undefined ? makeInitialState() : arguments[0];
	    ...
	    switch (action.type) {
	        ...
	        case 'TICK':
	            ...
	            var shifted_down = state.current.shifted(0, 1);
	            if (colliding(shifted_down, state.board)) {
	                var _clearRows = clearRows(state.current.spaces.concat(state.board));

	                var new_board = _clearRows.board;
	                var points_awarded = _clearRows.points;
